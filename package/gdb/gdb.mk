...
GDB_MAKE_OPT += MAKEINFO=true
GDB_INSTALL_TARGET_OPT += MAKEINFO=true DESTDIR=$(TARGET_DIR) install
HOST_GDB_MAKE_OPT += MAKEINFO=true
HOST_GDB_INSTALL_OPT += MAKEINFO=true install
...
# be built as C++ app.
ifeq ($(BR2_arc),y)
GDB_CONF_OPT += --disable-build-with-cxx
endif
...
GDB_CONF_OPT += --disable-inprocess-agent
endif
...
GDB_CONF_OPT += --with-expat
GDB_CONF_OPT += --with-libexpat-prefix=$(STAGING_DIR)/usr
GDB_DEPENDENCIES += expat
else
GDB_CONF_OPT += --without-expat
endif

ifeq ($(BR2_PACKAGE_XZ),y)
GDB_CONF_OPT += --with-lzma
GDB_CONF_OPT += --with-liblzma-prefix=$(STAGING_DIR)/usr
GDB_DEPENDENCIES += xz
else
GDB_CONF_OPT += --without-lzma
endif

ifeq ($(BR2_PACKAGE_ZLIB),y)
GDB_CONF_OPT += --with-zlib
GDB_DEPENDENCIES += zlib
else
GDB_CONF_OPT += --without-zlib
endif
...
HOST_GDB_CONF_OPT += --with-python=$(HOST_DIR)/usr/bin/python2
...
HOST_GDB_CONF_OPT += --enable-sim
else
HOST_GDB_CONF_OPT += --disable-sim
endif
